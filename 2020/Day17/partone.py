...
def expand_in_all_directions(array):
    new_array = np.zeros((array.shape[0] + 2, array.shape[1] + 2, array.shape[2] + 2))
    new_array[1:-1,1:-1,1:-1] = array
    return new_array

def count_active_neighbors(array):
    # Plan is to create 26 arrays, where each is a copy of the original,
    ...
    directions = np.indices((3,3,3)) - 1
    directions = np.moveaxis(directions, 0, 3)
    ...
    for i in np.arange(directions.shape[0]):
        neighbor_array = np.roll(num_array, (directions[i][0],directions[i][1],directions[i][2]), (0,1,2))
...
if __name__ == "__main__":
    ...
    input_array = np.full((1, len(input_lines), len(input_lines[0]) - 1), '.', dtype=str)

    for i in range(len(input_lines)):
        for j in range(len(input_lines[0]) - 1):
            input_array[0,i,j] = input_lines[i][j]
